...
def _accuracy_banner_html(accuracy_log: dict) -> str:
    """Render a backtest accuracy summary banner for the homepage."""
    if not accuracy_log:
        return ""
    total = len(accuracy_log)
    correct = sum(1 for r in accuracy_log.values() if r["correct"].lower() == "true")
    pct = round(correct / total * 100)
    items = ""
    for row in accuracy_log.values():
  ...
  <div style="font-size:1rem; font-weight:600; color:#fff; margin-bottom:8px;">
    🎯 Backtest Accuracy: {correct}/{total} race winners correctly predicted ({pct}%)
...
def build_index(races: list[dict], accuracy_log: dict) -> None:
    ...
    for year in years:
        ...
        sections_html += f"<h2>{year_label}</h2>\n{_race_cards_html(year_races)}\n"

    accuracy_html = _accuracy_banner_html(accuracy_log)
