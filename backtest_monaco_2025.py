#!/usr/bin/env python3
"""
Backtest: FORMULA 1 TAG HEUER GRAND PRIX DE MONACO 2025
=========================================================
Runs every stage of the Monaco 2025 weekend using only data that
would have been available at that point in time.

Circuit: Circuit de Monaco, Monte Carlo, Monaco
Date:    23–25 May 2025 (STANDARD WEEKEND)
           FP1 + FP2:        Friday 23 May
           FP3 + Qualifying: Saturday 24 May
           Race:             Sunday 25 May
Round:   8 of 24, 16 races remaining after Monaco

This backtest exercises all implemented features:
  ✓ Per-session weather from Open-Meteo archive (Monaco May — variable)
  ...
  ✓ Active-driver filter using 2025 Monaco grid (DOO, not COL — Colapinto joined Round 9)
  ✓ Standard weekend format (FP1 → FP2 → FP3 → Qualifying → Race)
  ✓ New-driver synthetic entries (ANT/BOR/HAD/DOO/BEA/LAW — no prior Monaco history)
...
Reports saved to: reports/monaco_2025/

Usage:
    uv run python backtest_monaco_2025.py
"""
...
RACE_SLUG  = "monaco"
YEAR       = 2025
RACE_DATE  = pd.Timestamp("2025-05-25")
RACE_NAME  = "Monaco Grand Prix"
LLM        = "openai:gpt-4o-mini"

ROUND_NUMBER    = 8           # Monaco was Round 8 in 2025
RACES_BEFORE    = 7           # rounds completed before Monaco (rounds 1–7)
REMAINING_PRE   = 24 - RACES_BEFORE   # = 17 (Monaco + 16 after, for pre-race forecast)
REMAINING_POST  = 24 - ROUND_NUMBER   # = 16 (races after Monaco, for post-race forecast)

# ── 2025 Monaco driver grid ───────────────────────────────────────────────────
# Key note: Jack Doohan (DOO) drove for Alpine at Monaco (Round 8).
# Colapinto (COL) replaced him from Round 9 onward.
# Key transfers vs 2024: HAM→Ferrari, SAI→Williams, ANT→Mercedes (rookie),
#                        TSU→Red Bull, LAW+HAD→Racing Bulls, OCO→Haas,
#                        BOR→Kick Sauber (rookie), DOO→Alpine (rookie).
...
DRIVERS_2025: dict[str, str] = {
    ...
    "ALO": "Aston Martin",
    "STR": "Aston Martin",
    # Alpine — Doohan raced Monaco (Round 8); Colapinto replaced him from Round 9
    "GAS": "Alpine",
...
# ── Step 1: Inject Monaco 2025 into CalendarManager ──────────────────────────
# CalendarManager only knows 2026 races.  Inject a 2025 Monaco entry so that
...
_MONACO_2025 = Race(
    ...
    # Standard weekend: FP1+FP2 Fri 23 May, FP3+Qualifying Sat 24 May, Race Sun 25 May
    sessions=_make_standard_weekend(date(2025, 5, 25)),
)


class _BacktestCalendar(CalendarManager):
    """CalendarManager subclass that resolves 'monaco' to the 2025 entry."""
...
def _build_championship_data(
    ...
    cutoff=RACE_DATE  → includes 2025 rounds 1–7 only (pre-Monaco simulation)
    cutoff=None       → includes all 2025 rounds through Monaco (round 8)
    """
...
def _run_stage(
    ...
    print(f"\n  ✅ Stage '{stage}' complete — reports/monaco_2025/")
    return race_fc
...
def main() -> None:
    print("\n" + "=" * 60)
    print("  BACKTEST: MONACO GRAND PRIX 2025 (STANDARD WEEKEND)")
    print("  Circuit de Monaco, 23–25 May 2025")
    print("  Round 8/24 — 16 races remaining after Monaco")
    ...
    # Monaco in May can be variable — the 1984 rain-shortened race is legendary.
    # Fetch archive data for each actual session date so the LLM query gets
    # session-level weather context.
    print("\n🌦  Fetching per-session weather from Open-Meteo archive (Monaco 2025)...")
    weekend_weather = fetch_weekend_weather(
        circuit_slug=RACE_SLUG,
        race_date=date(2025, 5, 25),
        session_dates={
            "FP1":        date(2025, 5, 23),  # Friday
            "FP2":        date(2025, 5, 23),  # Friday
            "Qualifying": date(2025, 5, 24),  # Saturday
            "Race":       date(2025, 5, 25),  # Sunday
        },
    )
    ...
    print("\n📦 Building Monaco circuit series (2010–2024)...")
    ...
    # Drivers with no Monaco Grand Prix history (2010–2024) get synthetic entries
    # so TimeCopilot can still forecast for them with an estimated baseline position.
    #
    # Included (no Monaco race history):
    #   ANT — 2025 rookie, never raced Monaco
    #   BOR — 2025 rookie, never raced Monaco
    #   HAD — 2025 rookie, never raced Monaco
    #   DOO — 2025 rookie, never raced Monaco (Doohan; Colapinto not yet in team)
    #   BEA — joined F1 mid-2024 (Monza = Round 16); Monaco 2024 was Round 8 → no history
    #   LAW — joined F1 at Singapore 2023 (Round 15) and Singapore 2024 (Round 18);
    #          Monaco is always earlier in the calendar than Singapore
    #
    # Excluded (have prior Monaco circuit history):
    #   All other 2025 Monaco drivers raced Monaco in prior years
    _NEW_DRIVERS_2025 = {
        "ANT": {"constructor": "Mercedes",     "driver_name": "Antonelli", "estimated_position": 12.0},
        "BOR": {"constructor": "Kick Sauber",  "driver_name": "Bortoleto", "estimated_position": 18.0},
        "HAD": {"constructor": "Racing Bulls", "driver_name": "Hadjar",    "estimated_position": 14.0},
        "DOO": {"constructor": "Alpine",       "driver_name": "Doohan",    "estimated_position": 16.0},
        "BEA": {"constructor": "Haas",         "driver_name": "Bearman",   "estimated_position": 16.0},
        "LAW": {"constructor": "Racing Bulls", "driver_name": "Lawson",    "estimated_position": 13.0},
    }

    # Constructor championship standings at Round 7 (last round before Monaco)
    # → gives per-constructor car_pace_rank reflecting 2025 early-season competitiveness
    print("\n📦 Fetching constructor standings at Round 7 (pre-Monaco)...")
    ...
    #   active_drivers    = 2025 Monaco grid (includes DOO, not COL — 7 races completed before Monaco)
    #   current_team_map  = 2025 assignments (HAM→Ferrari, SAI→Williams, etc.)
    #   car_pace_rank_map = per-constructor championship position at Round 7
    #   new_driver_entries = rookies/newcomers with no Monaco circuit history
    ...
    if circuit_weather_enc == 1:
        ...
        if not stats.empty:
            print("\n🌧  Wet driver advantage at Monaco (from historical circuit data):")
    ...
    # Only forecast championship for the 2025 Monaco grid.
    ...
    # Stage 6: Post-race championship — uses full 2025 standings through Monaco
    print("\n📦 Building post-race championship series (2015–2025 through Monaco)...")
    ...
    log_race_weather(
        ...
        circuit_type="street",
        race_date=date(2025, 5, 25),
        qualifying_date=date(2025, 5, 24),
    )
    ...
    print(f"  Reports → reports/monaco_2025/")
    print()
